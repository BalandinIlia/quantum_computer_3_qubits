...
}
{
    ...
    have repl: ((oracle f • OP stateBefore stateBefore) • HC.adj (oracle f)) = OP (stateAfter f) (stateAfter f) := by
        ...
        all_goals simp [oracleCore, OU.o3ByCore, FS.FS8, OP, IP.smulLeft, IP.smulRight, IP.distrLeft, IP.distrRight, OU.o3AdjByCore, stateBefore, stateAfter, stateBeforeRaw, stateAfterRaw, tmp, TensorProduct.add_tmul, TensorProduct.tmul_add]
        all_goals simp [eq]
        all_goals apply OU.Equality3S
        all_goals intro v1 v2 v3
        all_goals simp [IP.smulLeft, IP.smulRight, IP.distrLeft, IP.distrRight]
        all_goals fin_cases v1
        all_goals fin_cases v2
        all_goals fin_cases v3
        all_goals simp [eq]
