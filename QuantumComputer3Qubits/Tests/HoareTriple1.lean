...
Inf  (Cond.c2 (OP
     ...
     (Prog.ass1 (CS.qi 1 2))
     (Cond.c3 (OP CS.s001 CS.s001)) := by
  let st := Inf.Ax.Inf_2_1 0 1
        (by aesop)
        2
        (by aesop)
        (by aesop)
  ...
  have repr:
    ...
    clear st
    let sta:= LER.reg2ireg1i_reg3 0 1 (by aesop) 2 (by aesop) (by aesop) (TensorProduct.tmul ℂ (CS.qqi 0 0 0 1 (by aesop)) (CS.qi 1 2))
    have t1: ((TO.tpo2o1i 0 1 (by aesop) 2 (by aesop) (by aesop)) (OP (CS.qqi 0 0 0 1 (by aesop)) (CS.qqi 0 0 0 1 (by aesop)))) (OP (CS.qi 1 2) (CS.qi 1 2)) = OP sta sta := by
      simp only [sta]
      clear sta
      apply OP.Equality3
      intro v1 v2 v3
